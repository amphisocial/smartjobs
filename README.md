...
- **Speaking indicator:** while speaking, a soft glow ring pulses around the interviewer photo (+ the flickering speaker icon). For REAL open/close, drop in `interviewer-male-open.jpg` and `interviewer-female-open.jpg` (same person, mouth open) — the app auto-detects them and swaps between closed/open frames while speaking.
