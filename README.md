...
- **Interviewer:** cartoon male/female avatars (inline SVG) with a directly-animated mouth that opens and closes while speaking, a speaking glow ring, and the flickering speaker icon. The mouth is a `.mouth` SVG group animated via the `mouthtalk` keyframes — fully controllable, no photos needed.
