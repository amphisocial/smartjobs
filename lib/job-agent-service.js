...
function normalizePlan(raw, agent) {
  ...
  return {
    ...
    queries,
    sourcePolicy: fallback.sourcePolicy,
...
function fallbackEvaluation(agent, job) {
  ...
  if (job.official_source) score += 4;
  if (job.application_open_verified) score += 3;
  if (!job.date_posted && agent.posting_date_policy === "allow_missing") score -= 4;
  if (job.repost_detected && agent.repost_policy === "use_original") score -= 2;
  ...
  return {
    ...
    whyIncludedOrExcluded: `Deterministic fallback evaluation used because the AI evaluation was unavailable. Source=${job.source_system || "employer"}; posting date=${job.date_posted || "unavailable"}; application open=${job.application_open_verified ? "verified" : "not explicit"}.`,
  };
}
...
export async function runJobSearchAgent({ owner, agentId, triggerType = "manual", onProgress = () => {} }) {
  ...
  const errors = [];
  let stats = { queryCount: 0, discoveredCount: 0, verifiedCount: 0, recommendedCount: 0, skippedCount: 0, emptyQueryCount: 0, providerDiagnostics: {}, rejectionReasons: {} };
  try {
    ...
    const discovery = await discoverAndVerifyJobs(plan, agent, onProgress);
    errors.push(...discovery.errors);
    stats.discoveredCount = discovery.discoveredCount;
    stats.verifiedCount = discovery.verified.length;
    stats.skippedCount = discovery.skippedCount || 0;
    stats.emptyQueryCount = discovery.emptyQueryCount || 0;
    stats.providerDiagnostics = discovery.diagnostics || {};
    stats.rejectionReasons = discovery.rejectionReasons || {};

    if (!stats.discoveredCount) {
      throw new Error("Search discovery returned zero candidates. Use Test search connection and check SERPER_API_KEY or BRAVE_SEARCH_API_KEY.");
    }
