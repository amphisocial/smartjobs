...
import { verifyGoogleCredential, createRecruiterSession, verifyRecruiterSession } from "./google-auth.js";
import * as RP from "./recruiter-prompts.js";
import {
  initRecruiterStore, recruiterDbReady, recruiterOwnerKey, migrateRecruiterOwner, upsertRecruiterAccount,
  getDailyUsage, consumeDailyUsage, createJob, listJobs, getJob, updateJob,
...
const LIMITS = {
  job_create: () => config.recruiterFreeJobsDaily,
  rank_run: () => config.recruiterFreeRankRunsDaily,
  interview_start: () => config.recruiterFreeInterviewsDaily,
};
...
function normalizeJob(d = {}, fallback = {}) {
  return {
    ...
    startDate: /^\d{4}-\d{2}-\d{2}$/.test(String(fallback.startDate || d.startDate || ""))
      ? String(fallback.startDate || d.startDate) : null,
...
function limitPayload(used, unlimited) {
  const row = (key, limit) => ({
    used: unlimited ? 0 : Number(used[key] || 0),
    limit,
    remaining: unlimited ? null : Math.max(0, limit - Number(used[key] || 0)),
  });
  return {
    jobs: row("job_create", config.recruiterFreeJobsDaily),
    ranks: row("rank_run", config.recruiterFreeRankRunsDaily),
    interviews: row("interview_start", config.recruiterFreeInterviewsDaily),
  };
}
async function usageFor(a) {
  return limitPayload(a.unlimited ? {} : await getDailyUsage(a.owner), a.unlimited);
}
async function send(res, a, payload = {}) {
  res.json({
    ...payload,
    user: a.user,
    unlimited: a.unlimited,
    usage: await usageFor(a),
  });
}
async function consume(a, res, action) {
  if (a.unlimited) return true;
  const result = await consumeDailyUsage(a.owner, action, LIMITS[action]());
  if (result.allowed) return true;
  res.status(429).json({
    error: "recruiter_daily_limit",
    message: `Daily free limit reached for ${action === "job_create" ? "new jobs" : action === "rank_run" ? "candidate ranking" : "interviews"}.`,
    action,
    limit: result.limit,
    unlimited: false,
    usage: await usageFor(a),
...
function knownError(res, err) {
  ...
  if (err.message === "recruiter_database_not_configured") {
    return res.status(503).json({ error: "Recruiter workspace requires DATABASE_URL and the recruiter schema." });
  }
  if (err.message === "google_auth_not_configured") {
    return res.status(503).json({ error: "Google authentication is not configured." });
  }
  if (/missing_google_credential|invalid_google_identity|Invalid token|Wrong recipient/i.test(err.message)) {
    return res.status(401).json({ error: "Google sign-in could not be verified. Please sign in again." });
  }
  if (/valid URL|http\(s\)|private address|resolve/.test(err.message)) {
    return res.status(400).json({ error: err.message });
  }
  if (/429|quota|RESOURCE_EXHAUSTED|insufficient_quota/i.test(err.message)) {
...
async function safeUrl(raw) {
  ...
  const addrs = await dns.lookup(u.hostname, { all: true })
    .catch(() => { throw new Error("Couldn't resolve that link."); });
  if (addrs.some(a => isPrivateIp(a.address))) {
    throw new Error("That link points to a private address and can't be fetched.");
  }
  return u.toString();
}
...
async function fetchReadable(url) {
  ...
  try {
    const r = await fetch(safe, {
      ...
      redirect: "follow",
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; SmartJobsRecruiter/1.0)",
...
async function auth(req, res) {
  if (!recruiterDbReady()) {
    res.status(503).json({ error: "Recruiter workspace requires PostgreSQL. Install db/recruiter_schema.sql and set DATABASE_URL." });
    return null;
  }
  const user = verifyRecruiterSession(String(req.body?.recruiterSession || "").trim());
  if (!user) {
    res.status(401).json({ error: "google_signin_required", message: "Sign in with Google to access the recruiter workspace." });
    return null;
  }
  ...
  if (memberToken) {
    ...
    unlimited = Boolean(member && ["active", "trialing"].includes(member.status));
    if (unlimited) {
      // Preserve jobs created by the previous member-code-only recruiter workspace.
...
export async function installRecruiterRoutes(app) {
  ...
  app.get("/api/recruiter/auth/config", (_req, res) => {
    res.json({
      enabled: googleAuthEnabled(),
      clientId: googleAuthEnabled() ? config.googleClientId : "",
      limits: {
  ...
  app.post("/api/recruiter/auth/google", async (req, res) => {
    try {
      if (!recruiterDbReady()) {
        return res.status(503).json({ error: "Recruiter workspace requires PostgreSQL before Google sign-in can be completed." });
      }
      const user = await verifyGoogleCredential(req.body?.credential);
      const owner = recruiterOwnerKey(`google:${user.sub}`);
      await upsertRecruiterAccount(owner, user);
      res.json({
        sessionToken: createRecruiterSession(user),
  ...
  app.post("/api/recruiter/bootstrap", async (req, res) => {
    try {
      ...
      await send(res, a, {
        jobs: await listJobs(a.owner),
        pairs: await listInterviewPairs(a.owner),
      });
  ...
  app.post("/api/recruiter/jobs/list", async (req, res) => {
    try {
      const a = await auth(req, res); if (!a) return;
      await send(res, a, { jobs: await listJobs(a.owner) });
    } catch (e) { knownError(res, e); }
  });
  ...
  app.post("/api/recruiter/jobs/get", async (req, res) => {
    try {
      const a = await auth(req, res); if (!a) return;
      const job = await getJob(a.owner, req.body.jobId);
      if (!job) return res.status(404).json({ error: "Job not found." });
      await send(res, a, { job });
    } catch (e) { knownError(res, e); }
  });
  ...
  app.post("/api/recruiter/jobs/create", async (req, res) => {
    try {
      ...
      if (!raw && !req.body.roleDescription) {
        return res.status(400).json({ error: "Paste a job description or use AI Help." });
      }
      if (!await consume(a, res, "job_create")) return;
    ...
    try {
      ...
      if (text.length < 120) {
        const note = sourceType === "linkedin"
          ? "LinkedIn commonly blocks server-side retrieval. Paste the visible job/profile content and import again."
  ...
  app.post("/api/recruiter/candidates/add", async (req, res) => {
    try {
      ...
      if (!req.body.jobId || !String(c.name || "").trim() || String(c.resumeText || "").trim().length < 40) {
        return res.status(400).json({ error: "Candidate name and readable resume are required." });
      }
      ...
      await send(res, a, {
        candidate,
        job: await getJob(a.owner, req.body.jobId),
      });
    } catch (e) { knownError(res, e); }
  });
  ...
  app.post("/api/recruiter/candidates/pipeline", async (req, res) => {
    try {
      ...
      if (!allowed.includes(req.body.pipelineStatus)) {
        return res.status(400).json({ error: "Invalid pipeline status." });
      }
      const row = await updateCandidatePipeline(
        a.owner,
        a.actor,
        req.body.jobId,
        req.body.candidateId,
        req.body.pipelineStatus,
        req.body.notes || "",
      );
      if (!row) return res.status(404).json({ error: "Candidate/job link not found." });
      await send(res, a, { ok: true });
    } catch (e) { knownError(res, e); }
  });
    ...
    try {
      ...
      if (!targets.length) {
        await send(res, a, {
          job,
          rankedNow: 0,
          message: "All candidates already have a current ranking.",
        });
        return;
      }
      // One click/batch is one ranking run, regardless of candidate count.
      if (!await consume(a, res, "rank_run")) return;
      ...
      await send(res, a, { job, rankedNow: targets.length });
    } catch (e) { knownError(res, e); }
  });
  ...
  app.post("/api/recruiter/job-agent/chat", async (req, res) => {
    try {
      ...
      await addJobBuilderMessage(session.id, "assistant", out.message || "", {
        draft: out.draft || {},
        ready: !!out.ready,
      });
      await send(res, a, {
  ...
  app.post("/api/recruiter/interviews/list", async (req, res) => {
    try {
      const a = await auth(req, res); if (!a) return;
      await send(res, a, {
        pairs: await listInterviewPairs(
          a.owner,
          req.body.jobId || null,
  ...
  app.post("/api/recruiter/interviews/start", async (req, res) => {
    try {
      ...
      if (!await consume(a, res, "interview_start")) return;
      ...
      await addInterviewTurn(
        session.id,
        "system",
        out.coachWelcome || "Practice session started.",
        {
          coverage: out.coverage || {},
          riskAreas: out.riskAreas || [],
          suggestedNextQuestion: out.suggestedFirstQuestion || "",
        },
      );
      await addInterviewTurn(
        session.id,
        "candidate",
        out.candidateOpening || "Thanks for taking the time to speak with me.",
      );
      await send(res, a, { sessionId: session.id, pair, ...out });
    } catch (e) { knownError(res, e); }
  });
  ...
  app.post("/api/recruiter/interviews/turn", async (req, res) => {
    try {
      ...
      await addInterviewTurn(session.id, "candidate", out.candidateAnswer || "", {
        coverage: out.coverage || session.coverage,
      });
      await addInterviewTurn(session.id, "coach", out.questionAssessment || "", {
        coverage: out.coverage || session.coverage,
        unresolvedRisks: out.unresolvedRisks || [],
        suggestedNextQuestion: out.suggestedNextQuestion || "",
        questionQualityScore: out.questionQualityScore,
      });
      await send(res, a, out);
    } catch (e) { knownError(res, e); }
  });
  ...
  app.post("/api/recruiter/interviews/finish", async (req, res) => {
    try {
      ...
      if (session.status === "completed") {
        await send(res, a, { summary: session.summary });
        return;
      }
      const pair = await getCandidateJob(a.owner, session.job_id, session.candidate_id);
