...
export function searchPlanPrompt(agent) {
  return {
    ...
    user: `APPLICANT SEARCH PROFILE:\n${safeJson({
      ...
      maxPostingAgeDays: agent.max_posting_age_days,
      postingDatePolicy: agent.posting_date_policy,
      repostPolicy: agent.repost_policy,
      officialSourcesOnly: agent.official_sources_only,
      verifyApplicationOpen: agent.verify_application_open,
      allowAggregatorDiscovery: agent.allow_aggregator_discovery,
      preferredSourceSystems: agent.preferred_source_systems,
...
export function fitEvaluationPrompt(agent, jobs) {
  const compactJobs = jobs.map((job, index) => ({
    ...
    originalDatePosted: job.original_date_posted,
    postingDateSource: job.posting_date_source,
    repostDetected: job.repost_detected,
    sourceSystem: job.source_system,
    officialSource: job.official_source,
    applicationOpenVerified: job.application_open_verified,
