...
import { diagnoseJobSearchProvider, jobSearchProviderStatus } from "./job-search-engine.js";
...
export async function installJobAgentRoutes(app) {
  ...
  const providerStatus = jobSearchProviderStatus();
  console.log(`[job-agent] search provider=${providerStatus.requested} serper=${providerStatus.serperConfigured ? `configured (${providerStatus.serperKeySource})` : "missing"} brave=${providerStatus.braveConfigured ? `configured (${providerStatus.braveKeySource})` : "missing"} fallback=${providerStatus.allowFallback}`);
  ...
  app.get("/api/job-agent/config", (_req, res) => {
    res.json({
      ...
      searchProviderStatus: jobSearchProviderStatus(),
      serperConfigured: Boolean(config.serperApiKey),
      serperKeySource: config.serperKeySource,
      braveConfigured: Boolean(config.braveSearchApiKey),
      braveKeySource: config.braveKeySource,
    });
  });
